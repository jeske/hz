...
{ 
 ...
 /* Get alpha */
 a=(Sint32)(color & (Uint32)0x000000ff);
...
/* Clipping based heavily off of code from                   */
...
fixed-point arithmetic. By A. Schiffler.

*/

static Uint8 *gfxPrimitivesWeighttableA=NULL;
static Uint8 *gfxPrimitivesWeighttableB=NULL;
...
int aalineColor (SDL_Surface *dst, Sint16 x1, Sint16 y1, Sint16 x2, Sint16 y2, Uint32 color)
{
 ...
 /* Calculate table of alpha weights if required */  
 if (gfxPrimitivesWeighttableA==NULL) {
   /* Allocate memory */
   gfxPrimitivesWeighttableA=malloc(256);
   gfxPrimitivesWeighttableB=malloc(256);
   /* Calculate coefficient */
   coeffTable=1.0/1.4142;
   /* Fill table */
   curTableA=gfxPrimitivesWeighttableA;
   curTableB=gfxPrimitivesWeighttableB;
   for (i=0; i<256; i++) {
    *curTableA=(Uint8)((float)i*coeffTable);
    *curTableB=(Uint8)((float)(255-i)*coeffTable);
    curTableA++;
    curTableB++;
   }
 }
 ...
 if (dx==0) {
  ...
  if (dy > dx) {
	...
	x0pxdir += xdir;
	 ...
	 the paired pixel. */
      wgt = erracc >> intshift;      
      result |= pixelColorWeight (dst, xx0, yy0, color, gfxPrimitivesWeighttableA[wgt]);
      result |= pixelColorWeight (dst, x0pxdir, yy0, color, gfxPrimitivesWeighttableB[wgt]);      
    }
  ...
  while (--dx) {
    ...
    wgt = erracc >> intshift;
    result |= pixelColorWeight (dst, xx0, yy0, color, gfxPrimitivesWeighttableA[wgt]);
    result |= pixelColorWeight (dst, xx0, y0p1, color, gfxPrimitivesWeighttableB[wgt]);
...
/* with multiple-draw removal and other speedup changes.               */
...
static SDL_Surface *gfxPrimitivesFont=NULL;
...
int characterColor (SDL_Surface *dst, Sint16 x, Sint16 y, char c, Uint32 color)
{
 ...
 /* Create 8x8 bitmap surface if not already present */
 if (gfxPrimitivesFont==NULL) {
  gfxPrimitivesFont=SDL_CreateRGBSurface(SDL_SWSURFACE | SDL_HWSURFACE | SDL_SRCALPHA, 8, 8, 32, 0xFF000000, 0x00FF0000, 0x0000FF00, 0x000000FF);
  SDL_SetAlpha (gfxPrimitivesFont, SDL_SRCALPHA, 255);
 }
 ...
 /* Clear bitmap */
 curpos=(Uint8 *)gfxPrimitivesFont->pixels;
 ...
 result=SDL_BlitSurface (gfxPrimitivesFont, &srect, dst, &drect);
